...
hts_util::contig2map_map hts_util::bcf_to_map(vcfFile* vcf_fp, bcf_hdr_t* vcf_hdr) {
...
hts_util::VarList hts_util::bam_to_vars(bam1_t* aln) {
    VarList vs;
...
hts_util::VarList hts_util::bcf_to_vars(bcf1_t* b) {
    VarList vs;
    ...
    for (uint32_t i = 1; i < b->n_allele; ++i) {
        ...
        if (strlen(alt)  < strlen(ref)) { // DEL
            ...
            vs.push_back(Var(b->pos, hts_util::VTYPE::V_INS, alt, ref, b->d.id)); // don't need ref here
        } else { // SNP
...
void hts_util::print_varlist(hts_util::VarList vs, FILE* out) {
    for (const auto& v: vs) {
